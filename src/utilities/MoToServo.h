...
class MoToServo
{
  private:
    ...
    void setSpeed(uint16_t);      // Set movement speed, the higher the faster
                             // Zero means no speed control (default)
    void setSpeed(uint16_t,bool); // Set compatibility-Flag (true= compatibility with version V08 and earlier)
    #define HIGHRES 0
    #define SPEEDV08 1
